...
def init_game(lineup_1, lineup_2):
    ...
    t1_gain = gr.Number(label="Team 1 gain", value=25-min(25, abs(ELO_1-ELO_2)//100)+1, visible=True)
    t2_gain = gr.Number(label="Team 2 gain", value=25+min(25, abs(ELO_1-ELO_2)//100), visible=True)
...
def save_match_history(team_1_result, team_2_result):
    ...
    team_1_result.to_csv(f'./match_history/match_{match_num+1}/t1.csv')
    team_2_result.to_csv(f'./match_history/match_{match_num+1}/t2.csv')


def submit_match(lineup_1, lineup_2, result_1, result_2, t1_gain, t2_gain, win_team):
    players_1 = lineup_1.split(",")
    players_2 = lineup_2.split(",")
    if win_team == "Team 1":
        elo = t1_gain
...
if __name__ == '__main__':
    with gr.Blocks() as app:
        ...
        create_button.click(init_game, [lineup_1, lineup_2], [team_1, team_2, elo_diff, t1_gain, t2_gain, win_team])
        submit_button.click(submit_match, [lineup_1, lineup_2, team_1_result, team_2_result, t1_gain, t2_gain, win_team], [database, top_1, top_2, top_3])
